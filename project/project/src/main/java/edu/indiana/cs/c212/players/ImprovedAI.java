...
public class ImprovedAI extends AbstractPlayer {
	...
	public ImprovedAI (PlayerColor player){
			super(player);
	...
	public Move RedStrat(Board board, List<Move> legalMoves) {
		...
		if (counter == 1){
			for (int i = 0; i < board.getSize(); i++) {
				...
				if(i + 3 < board.getSize()){
					Move move3 = new Move(i + 3,j);
		...
		if(second){
			int x = lastMove.getX();
			int y = lastMove.getY();
			Move secondmove = new Move(x ,y - 1);
			Move secondmove2 = new Move(x,y + 1);
